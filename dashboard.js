...
// Todas as datas entre inicio e fim (inclusive), em ordem — apenas dias úteis (seg a sex),
// já que os aulões só acontecem de segunda a sexta.
...
async function renderizarDias(dias, labelTexto) {
  ...
  if (dias.length === 0) {
    tbody.innerHTML = `<tr><td colspan="3" style="text-align:center; color:var(--text-muted); padding:24px;">Nenhum dia útil (seg. a sex.) no período selecionado.</td></tr>`;
    return;
  }
  ...
  dias.forEach((d, idx) => {
    ...
    // Como só exibimos dias úteis (seg a sex), toda segunda-feira que não seja
    // o primeiro dia da lista marca o início de uma nova semana.
    if (idx > 0 && d.getDay() === 1) tr.classList.add("week-separator");

    const ehQuinta = d.getDay() === 4; // quinta-feira é dia de live no YouTube, não tem aulão
    if (ehQuinta) tr.classList.add("row-live");
    ...
    if (ehQuinta) {
      // Quinta-feira: campos travados, sempre "Live no YouTube" (não editável)
      const tdRespLive = document.createElement("td");
      tdRespLive.innerHTML = `<div class="live-locked-field">🔴 Live no YouTube</div>`;
      tr.appendChild(tdRespLive);

      const tdTemaLive = document.createElement("td");
      tdTemaLive.innerHTML = `<div class="live-locked-field">🔴 Live no YouTube</div>`;
      tr.appendChild(tdTemaLive);

      tbody.appendChild(tr);
      return; // pula a lógica de select/input abaixo
    }
