...
async function renderizarSemana() {
  ...
  document.getElementById("periodoChip").hidden = true;
...
async function renderizarPeriodo() {
  ...
  const chip = document.getElementById("periodoChip");
  chip.hidden = false;
...
function renderizarCalendario(gridEl, tituloEl, anchor) {
  ...
  dias.forEach((d) => {
    ...
    el.addEventListener("click", () => selecionarDataCalendario(new Date(d)));
    gridEl.appendChild(el);
  });
}
...
function configurarFiltroPeriodo() {
  ...
  document.querySelectorAll(".period-quick button").forEach((btn) => {
    btn.addEventListener("click", () => definirIntervaloRapido(btn.dataset.quick));
  });
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!popover.hidden && !popover.contains(e.target) && e.target.id !== "abrirPeriodoBtn") {
      fecharPopoverPeriodo();
    }
  });
}
